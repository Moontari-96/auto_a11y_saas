        ...
        const pages = await page.evaluate(dir => {
            ...
            return anchors
                .map(a => {
                    ...
                    // URL 객체로 변환하여 쿼리 스트링(?...)과 해시(#...)를 쉽게 분리
                    const fullUrl = new URL(a.href)
                    // origin + pathname만 합치면 쿼리 스트링이 제거된 순수 주소가 됩니다.
                    // 예: https://site.com/page.html?ref=News -> https://site.com/page.html
                    const cleanUrl = fullUrl.origin + fullUrl.pathname

                    return {
                        title: a.innerText.trim() || '제목 없음',
                        url: cleanUrl, // 👈 깨끗한 URL 저장
                    }
                })
                .filter(item => {
                    // 필터링 조건:
